...
rseStart() {
	if pgrep -f '^python.*/rseDaemon.py' >/dev/null; then
		...
		openwbDebugLog "MAIN" 0 "rse handler not running! starting process"
...
rseStop() {
	if pgrep -f '^python.*/rseDaemon.py' >/dev/null; then
		openwbDebugLog "MAIN" 0 "rse handler running but not configured. killing handler"
...
rseSetup() {
	...
	if ((forceRestart == 1)); then
		openwbDebugLog "MAIN" 0 "rse handler restart forced! killing handler"
		rseStop
	fi
	if ((enabled == 1)); then
		openwbDebugLog "MAIN" 0 "rse enabled"
		rseStart
	else
		openwbDebugLog "MAIN" 0 "rse disabled"
		rseStop
	fi
}
export -f rseSetup
