...
if [[ $evsecon == "dac" ]]; then
	sudo python /var/www/html/openWB/runs/dac.py 2206 $dacregister
fi
if [[ $evsecon == "modbusevse" ]]; then
	sudo python /var/www/html/openWB/runs/evsewritemodbus.py $modbusevsesource $modbusevseid 17
fi
if [[ $debug == "2" ]]; then
	...
	if [[ $evsecon == "simpleevsewifi" ]]; then
		...
		if (( current != 17 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp1 -s http://$evsewifiiplp1/setCurrent?current=17 > /dev/null
		fi
	fi
if [[ $lastmanagement == "1" ]]; then
	...
	if [[ $evsecons1 == "simpleevsewifi" ]]; then
		...
		current=$(echo $output | jq '.list[] | .actualCurrent')
		if (( current != 17 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp2 -s http://$evsewifiiplp2/setCurrent?current=17 > /dev/null
		fi
	fi
...
if [[ $lastmanagements2 == "1" ]]; then
	...
	if [[ $evsecons2 == "simpleevsewifi" ]]; then
		...
		current=$(echo $output | jq '.list[] | .actualCurrent')
		if (( current != 17 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp3 -s http://$evsewifiiplp3/setCurrent?current=17 > /dev/null
		fi
...
echo 17 > /var/www/html/openWB/ramdisk/llsoll
