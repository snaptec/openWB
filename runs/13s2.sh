...
. /var/www/html/openWB/openwb.conf
	if [[ $evsecons2 == "dac" ]]; then
		sudo python /var/www/html/openWB/runs/dac.py 1687 $dacregisters2
	fi

	if [[ $evsecons2 == "modbusevse" ]]; then
		sudo python /var/www/html/openWB/runs/evsewritemodbus.py $evsesources2 $evseids2 13
	fi
	if [[ $evsecons2 == "simpleevsewifi" ]]; then
		...
		if (( current != 13 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp3 -s http://$evsewifiiplp3/setCurrent?current=13 > /dev/null
		fi
	fi

echo 13 > /var/www/html/openWB/ramdisk/llsolls2
