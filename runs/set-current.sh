...
# function for Phoenix Connect ev
function setChargingCurrentIpPhoenix () {
current=$1
evseip=$2
# set desired charging current
sudo python /var/www/html/openWB/modules/phoenixlllp1/evsephoenixwritemodbus.py $current $evseip
}
...
function setChargingCurrent () {
	...
	if [[ $evsecon == "extopenwb" ]]; then
		setChargingCurrentExtopenwb $current $chargep1ip $chargecplp1
	fi
	...
	if [[ $evsecon == "ipphoenix" ]]; then
        setChargingCurrentIpPhoenix $current $phoenixlp1ip
    fi
}
...
if [[ $lastmanagementlp3 == "1" ]]; then
	if [[ $points == "all" ]] || [[ $points == "s2" ]]; then
		evsecon=$evseconlp3
		dacregister=$dacregisterlp3
		modbusevsesource=$modbusevsesourcelp3
		modbusevseid=$modbusevseidlp3
		...
		evseip=$evseiplp3
		ipevseid=$evseidlp3
		chargep1ip=$chargeiplp3
		chargecplp1=$chargecplp3
...
if [[ $lastmanagementlp4 == "1" ]]; then
	if [[ $points == "all" ]] || [[ $points == "lp4" ]]; then
		...
		evseip=$evseiplp4
		ipevseid=$evseidlp4
		chargep1ip=$chargeiplp4
		chargecplp1=$chargecplp4
...
if [[ $lastmanagementlp5 == "1" ]]; then
	if [[ $points == "all" ]] || [[ $points == "lp5" ]]; then
		...
		evseip=$evseiplp5
		ipevseid=$evseidlp5
		chargep1ip=$chargeiplp5
		chargecplp1=$chargecplp5
...
if [[ $lastmanagementlp6 == "1" ]]; then
	if [[ $points == "all" ]] || [[ $points == "lp6" ]]; then
		...
		evseip=$evseiplp6
		ipevseid=$evseidlp6
		chargep1ip=$chargeiplp6
		chargecplp1=$chargecplp6
...
if [[ $lastmanagementlp7 == "1" ]]; then
	if [[ $points == "all" ]] || [[ $points == "lp7" ]]; then
		...
		evseip=$evseiplp7
		ipevseid=$evseidlp7
		chargep1ip=$chargeiplp7
		chargecplp1=$chargecplp7
...
if [[ $lastmanagementlp8 == "1" ]]; then
	if [[ $points == "all" ]] || [[ $points == "lp8" ]]; then
		...
		evseip=$evseiplp8
		ipevseid=$evseidlp8
		chargep1ip=$chargeiplp8
		chargecplp1=$chargecplp8
