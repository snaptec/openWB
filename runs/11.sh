...
if [[ $debug == "2" ]]; then
	...
	if [[ $evsecon == "simpleevsewifi" ]]; then
		...
		if (( current != 11 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp1 -s http://$evsewifiiplp1/setCurrent?current=11 > /dev/null
		fi
	fi
if [[ $lastmanagement == "1" ]]; then
	...
	if [[ $evsecons1 == "simpleevsewifi" ]]; then
		...
		current=$(echo $output | jq '.list[] | .actualCurrent')
		if (( current != 11 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp2 -s http://$evsewifiiplp2/setCurrent?current=11 > /dev/null
		fi
	fi
	if [[ $evsecons1 == "modbusevse" ]]; then
		sudo python /var/www/html/openWB/runs/evsewritemodbus.py $evsesources1 $evseids1 11
	fi
	echo 11 > /var/www/html/openWB/ramdisk/llsolls1
...
if [[ $lastmanagements2 == "1" ]]; then
	...
	if [[ $evsecons2 == "simpleevsewifi" ]]; then
		...
		current=$(echo $output | jq '.list[] | .actualCurrent')
		if (( current != 11 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp3 -s http://$evsewifiiplp3/setCurrent?current=11 > /dev/null
		fi
