...
if [[ $debug == "2" ]]; then
	...
	if [[ $evsecon == "simpleevsewifi" ]]; then
		...
		if (( current != 31 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp1 -s http://$evsewifiiplp1/setCurrent?current=31 > /dev/null
		fi
	fi
...
if [[ $lastmanagements2 == "1" ]]; then
	...
	if [[ $evsecons2 == "simpleevsewifi" ]]; then
		...
		current=$(echo $output | jq '.list[] | .actualCurrent')
		if (( current != 31 )) ; then
			curl --silent --connect-timeout $evsewifitimeoutlp3 -s http://$evsewifiiplp3/setCurrent?current=31 > /dev/null
		fi
