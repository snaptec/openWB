...
sofortlademodus(){
	...
	if (( etprovideraktiv == 1 )); then
		...
		if (( $(echo "$actualprice <= $etprovidermaxprice" |bc -l) )); then
			...
			if [[ $debug == "1" ]]; then
				echo "Deaktiviere preisbasierte Ladung, Preis $actualprice, Max $etprovidermaxprice"
			fi
					...
					if (( ladeleistunglp1 < 100 )); then
						...
						if (( llalt > sofortll)); then
							...
							if (( llneu < minimalstromstaerke )); then
								...
								echo "$date LP1, Lademodus Sofort. Ladung geändert auf $llneu Ampere" >> ramdisk/ladestatus.log
								if [[ $debug == "1" ]]; then
									echo "Sofort ladung auf $llneu reduziert, unter eingestellter max A $minimalstromstaerke"
								fi
								exit 0
							fi
