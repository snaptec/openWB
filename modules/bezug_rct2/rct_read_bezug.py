...
def main():
    ...
    if clientsocket is not None:
        ...
        totalfeed=int(rct.read(clientsocket,0x44D4C533 )*-1.0) 
        #rct.dbglog("einspeisungkwh is "+ str(totalfeed))
        writeRam('einspeisungkwh', totalfeed, '0x44D4C533 energy.e_grid_feed_total')

 
        totalload=int(rct.read(clientsocket,0x62FBE7DC )) 
        #rct.dbglog("bezugkwh is "+ str(totalload))
        writeRam('bezugkwh',        totalload, '#0x62FBE7DC energy.e_grid_load_total')
                
        value = rct.read(clientsocket, 0x6002891F )
        writeRam('wattbezug', int(value )*1  , '#0x6002891F g_sync.p_ac_sc_sum')


        volt1=int(rct.read(clientsocket,0xCF053085 ))
        volt1=int(volt1 * 10) / 10.0
        #rct.dbglog("volt1 is "+ str(volt1))
        writeRam('evuv1', volt1, '0xCF053085 g_sync.u_l_rms[0] ')
        
        volt2=int(rct.read(clientsocket,0x54B4684E ))
        volt2=int(volt2 * 10) / 10.0
        #rct.dbglog("volt2 is "+ str(volt2))
        writeRam('evuv2', volt2, '0x54B4684E g_sync.u_l_rms[1] ')

        volt3=int(rct.read(clientsocket,0x2545E22D ))
        volt3=int(volt3 * 10) / 10.0
        #rct.dbglog("volt3 is "+ str(volt3))
        writeRam('evuv3', volt3, '0x2545E22D g_sync.u_l_rms[2] ')


        
        watt=int(rct.read(clientsocket,0x27BE51D9 ))
        ...
        watt=int(rct.read(clientsocket,0xF5584F90 ))
        ...
        watt=int(rct.read(clientsocket,0xB221BCFA ))
        ...
        freq=rct.read(clientsocket,0x1C4A665F )
        freq=int(freq * 100) / 100.0
        #rct.dbglog("freq is "+ str(freq))
        writeRam('evuhz', freq, '0x1C4A665F grid_pll[0].f')
        writeRam('llhz', freq, '0x1C4A665F grid_pll[0].f')


        stat1 = int(rct.read(clientsocket,0x37F9D5CA ))
        rct.dbglog("status1 "+ str(stat1))

        stat2 = int(rct.read(clientsocket,0x234B4736 ))
        rct.dbglog("status2 "+ str(stat2))

        stat3 = int(rct.read(clientsocket,0x3B7FCD47 ))
        rct.dbglog("status3 "+ str(stat3))
        
        stat4 = int(rct.read(clientsocket,0x7F813D73 ))
        rct.dbglog("status4 "+ str(stat4))
