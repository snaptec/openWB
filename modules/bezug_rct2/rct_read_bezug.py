...
def main(argv: List[str]):
    ...
    if clientsocket is not None:
        ...
        rct_lib.write_ramdisk('einspeisungkwh', totalfeed, '0x44D4C533 energy.e_grid_feed_total')
        rct_lib.write_ramdisk('bezugkwh',       totalload, '#0x62FBE7DC energy.e_grid_load_total')
        rct_lib.write_ramdisk('wattbezug', int(p_ac_sc_sum)*1, '#0x6002891F g_sync.p_ac_sc_sum')
        rct_lib.write_ramdisk('evuv1', volt1, '0xCF053085 g_sync.u_l_rms[0] ')
        rct_lib.write_ramdisk('evuv2', volt2, '0x54B4684E g_sync.u_l_rms[1] ')
        rct_lib.write_ramdisk('evuv3', volt3, '0x2545E22D g_sync.u_l_rms[2] ')

        rct_lib.write_ramdisk('bezugw1', watt1, '0x27BE51D9 als Watt g_sync.p_ac_sc[0]')
        ampere = int(watt1 / volt1 * 10.0) / 10.0
        rct_lib.write_ramdisk('bezuga1', ampere, '0x27BE51D9 als Ampere g_sync.p_ac_sc[0]')

        rct_lib.write_ramdisk('bezugw2', watt2, '0xF5584F90 als Watt g_sync.p_ac_sc[1]')
        ampere = int(watt2 / volt2 * 10.0) / 10.0
        rct_lib.write_ramdisk('bezuga2', ampere, '0xF5584F90 als Ampere g_sync.p_ac_sc[1]')

        rct_lib.write_ramdisk('bezugw3', watt3, '0xB221BCFA als Watt g_sync.p_ac_sc[2]')
        ampere = int(watt3 / volt3 * 10.0) / 10.0
        rct_lib.write_ramdisk('bezuga3', ampere, '0xF5584F90 als Ampere g_sync.p_ac_sc[2]')

        rct_lib.write_ramdisk('evuhz', freq, '0x1C4A665F grid_pll[0].f')
        rct_lib.write_ramdisk('llhz', freq, '0x1C4A665F grid_pll[0].f')
