...
def main():
    ...
    if clientsocket is not None:                             
        ...
        print "Battery Controler " + str(rct.host)
        Version = rct.read(clientsocket, 0x1B39A3A3) 
        print "Hardware Version     : " + str(Version)
        Version = rct.read(clientsocket, 0x9D785E8C) 
        print "Software Version     : " + str(Version)
        Ser = rct.read(clientsocket, 0x16A1F844) 
        print "Serial Nr            : " + str(Ser[74:88])

        Status =  rct.read(clientsocket, 0x70A2AF4F) 
        print "Status               : "  + str(Status)
        cap =  rct.read(clientsocket, 0xB57B59BD) 
        print "Max.Lade/Enlade      : "  + str(cap) + ' A'
        cyl =  rct.read(clientsocket, 0xC0DF2978) 
        print "Cycles               : "  + str(cyl)
        Eff =  int(rct.read(clientsocket, 0xACF7666B) * 10000) / 100.0
        print "Efficency            : "  + str(Eff) + ' %'
        Soh =  rct.read(clientsocket, 0x381B8BF9) 
        print "SoH                  : "  + str(Soh)
        SoC =  int(rct.read(clientsocket, 0x959930BF) *10000) / 100
        print "SoC                  : "  + str(SoC) + ' %'

        temp =  int(rct.read(clientsocket, 0x55DDF7BA) * 100) / 100.0
        print "Max Cell Temp.       : "  + str(temp) + ' Grad'

        ms1  =  rct.read(clientsocket, 0xFBF6D834) 
        if ms1[75] > ".":
            print "Batt.Pack 1          : "  + str(ms1[74:88])
        ms2  =  rct.read(clientsocket, 0x99396810) 
        if ms2[75] > ".":
            print "Batt.Pack 2          : "  + str(ms2[74:88])
        ms3  =  rct.read(clientsocket, 0x73489528) 
        if ms3[75] > ".":
            print "Batt.Pack 3          : "  + str(ms3[74:88])
        ms4  =  rct.read(clientsocket, 0x257B7612) 
        if ms4[75] > ".":
            print "Batt.Pack 4          : "  + str(ms4[74:88])
        ms5  =  rct.read(clientsocket, 0x4E699086) 
        if ms5[75] > ".":
            print "Batt.Pack 5          : "  + str(ms5[74:88])
        ms6  =  rct.read(clientsocket, 0x162491E8) 
        if ms6[75] > '.':
            print "Batt.Pack 6          : "  + str(ms6[74:88])

        Stat1=  rct.read(clientsocket, 0x71765BD8)
        print "Batt Status 1        : "  + str(Stat1)
        Stat2=  rct.read(clientsocket, 0x0DE3D20D)
