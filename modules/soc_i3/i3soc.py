...
def create_auth_string(client_id, client_password):
...
def authStage1(username, password, code_challenge, state):
    try:
...
def authStage2(authcode1, code_challenge, state):
    try:
        url = 'https://' + auth_server + '/gcdm/oauth/authenticate'
        headers = {
            'Content-Type': 'application/x-www-form-urlencoded',
...
def authStage3(authcode2, code_challenge):
    try:
