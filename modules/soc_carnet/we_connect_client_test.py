...
def getActualCurrent(session, url_base):
    try:
        estat = json.loads(CarNetPost(session, url_base, '/-/emanager/get-emanager'))
        #"settings":{"chargerMaxCurrent":13,"maxAmpere":16,"maxCurrentReduced":false}}
        print('{"errorCode":"0","chargerMaxCurrent":"' + str(estat.get('EManager').get('rbc').get('settings').get('chargerMaxCurrent')) + '"}')
        print('{"errorCode":"0","maxAmpere":"' + str(estat.get('EManager').get('rbc').get('settings').get('maxAmpere')) + '"}')
        print('{"errorCode":"0","maxCurrentReduced":"' + str(estat.get('EManager').get('rbc').get('settings').get('maxCurrentReduced')) + '"}')
    except:
        print('{"errorCode":"2","errorMsg":"Failed to get actual maximal charging currect."}')    
    return 0

def setActualCurrent(session, url_base, newValue):
    #"settings":{"chargerMaxCurrent":13,"maxAmpere":16,"maxCurrentReduced":false}}
    post_data = {
        'chargerMaxCurrent': int(newValue),
        'maxCurrentReduced': False
    }
    print(post_data)
    print(CarNetPostAction(session, url_base, '/-/emanager/charge-battery', post_data)) # URL/Message needs to be tested - re-engineered from Webinterface
    return 0        
...
if __name__ == '__main__':
    ...
    elif CARNET_COMMAND == 'getActualCurrent':
        getActualCurrent(session, url)
        #retrieveCarNetInfo(session, url)
    elif CARNET_COMMAND == 'setActualCurrent':
        setActualCurrent(session, url, 5)
