...
time_string = time.strftime("%m/%d/%Y, %H:%M:%S peugeotsoc lp1", named_tuple) 
...
f = open('/var/www/html/openWB/ramdisk/peugeotreq1lp1', 'w')
f.write(str(reg))
f.write(str(data))
f.write(str(headers))
f.close()
...
f = open('/var/www/html/openWB/ramdisk/peugeotreply1lp1', 'w')
...
f = open('/var/www/html/openWB/ramdisk/peugeotreq2lp1', 'w')
f.write(str(reg))
f.write(str(data))
f.write(str(headers))
f.close()
response=requests.get(reg,headers=headers )
f = open('/var/www/html/openWB/ramdisk/peugeotreply2lp1', 'w')
responsetext  = response.text
responestatus = response.status_code
f.write(str(responsetext))
f.write(str(responestatus))
...
headers = {'Accept':'application/hal+json','Authorization': 'Bearer %s' % acc_token,'x-introspect-realm ': realm}
f = open('/var/www/html/openWB/ramdisk/peugeotreq3lp1', 'w')
f.write(str(reg))
f.write(str(data))
f.write(str(headers))
f.close()
response=requests.get(reg,headers=headers )
f = open('/var/www/html/openWB/ramdisk/peugeotreply3lp1', 'w')
responsetext  = response.text
responestatus = response.status_code
f.write(str(responsetext))
f.write(str(responestatus))
