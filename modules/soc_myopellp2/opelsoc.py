...
time_string = time.strftime("%m/%d/%Y, %H:%M:%S opelsoc lp2", named_tuple) 
...
f = open('/var/www/html/openWB/ramdisk/opelreq1lp2', 'w')
f.write(str(reg))
f.write(str(data))
f.write(str(headers))
f.close()
...
f = open('/var/www/html/openWB/ramdisk/opelreply1lp2', 'w')
...
f = open('/var/www/html/openWB/ramdisk/opelreq2lp2', 'w')
f.write(str(reg))
f.write(str(data))
f.write(str(headers))
f.close()
response=requests.get(reg,headers=headers )
f = open('/var/www/html/openWB/ramdisk/opelreply2lp2', 'w')
responsetext  = response.text
responestatus = response.status_code
f.write(str(responsetext))
f.write(str(responestatus))
...
headers = {'Accept':'application/hal+json','Authorization': 'Bearer %s' % acc_token,'x-introspect-realm ': realm}
f = open('/var/www/html/openWB/ramdisk/opelreq3lp2', 'w')
f.write(str(reg))
f.write(str(data))
f.write(str(headers))
f.close()
response=requests.get(reg,headers=headers )
f = open('/var/www/html/openWB/ramdisk/opelreply3lp2', 'w')
responsetext  = response.text
responestatus = response.status_code
f.write(str(responsetext))
f.write(str(responestatus))
