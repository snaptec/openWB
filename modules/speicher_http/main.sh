...
bash "$OPENWBBASEDIR/packages/legacy_run.sh" "modules.devices.http.device" "bat" "${speicherleistung_http}" "${speicherikwh_http}" "${speicherekwh_http}" "${speichersoc_http}" >>"$MYLOGFILE" 2>&1
ret=$?
