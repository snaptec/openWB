...
#            |           |     |     |   +---------- Type (U32: unsigned int 32 bit (= default), I32: signed int 32 bit)
#            |           |     |     |   |       +-- Description
#            |           |     |     |   |       |
values = (('llv1',      4096, 0.001, 2, '',    'Voltage AC Phase 1 [0.001 * V]'),
          ...
          ('llkwh',     4124, 0.001, 2, '',    'Energy Real WAC Sum Consumed [0.001 * kWh]'),
          ...
          ('_sl123',    4120, 0.01,  0, '',    'Power Apparent S Sum [0.01 * VA]'),
          ...
          ('llpf1',     None, None,  2, '',    'Power Factor Phase 1'),
          ('llpf2',     None, None,  2, '',    'Power Factor Phase 2'),
          ('llpf3',     None, None,  2, '',    'Power Factor Phase 3'))
...
# Calculate internal values (here the power factor)
for i in (1, 2, 3):
    # cosphi = P/S = P/sqrt(Q^2+P^2)
    cosphi_num = value_dict['wl' + str(i)][0]
    cosphi_den = math.sqrt((value_dict['wl' + str(i)][0])**2 + (value_dict['_ql' + str(i)][0])**2)
    # Avoid a division by 0
    if cosphi_den != 0:
        value_dict['llpf' + str(i)] = [(cosphi_num/cosphi_den), ]
    else:
        value_dict['llpf' + str(i)] = [1, ]
