...
if (( timer < 60 )); then
 ...
 r9=$(</var/www/html/openWB/ramdisk/zoereply9lp1)
 scheduler=$(echo $r9 | jq -r .data.attributes.chargeMode)
# echo " $dtime zoe p1 lstate(wallbox) $lstate plugged(Wallbox) $plugstatus charging(Wallbox) $chagerstatus charging(Zoe) $charging scheduler(zoe) $scheduler soc $soc wakeupzoe $wakeupmyrenaultlp1 "
 if [[ $lstate == "1" ]] && [[ $chagerstatus == "0" ]] && [[ $plugstatus == "1" ]] && [[ $charging == '-1' ]] && [[ $scheduler == "always_charging" ]] && [[ $soc -ne 100 ]] && [[ $wakeupmyrenaultlp1 == "1" ]] ; then
      ...
      sudo python /var/www/html/openWB/modules/soc_myrenault/zoenwakelp1.py $myrenault_userlp1 $myrenault_passlp1  $myrenault_locationlp1  $myrenault_countrylp1
 fi
fi
