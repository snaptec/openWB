...
if (( timer < 60 )); then
 ...
 sudo python /var/www/html/openWB/modules/soc_myrenault/zoensoclp1.py $myrenault_userlp1 $myrenault_passlp1 $myrenault_locationlp1 $myrenault_countrylp1 $soc_vinlp1
 ...
 if [[ $lstate == "1" ]] && [[ $chagerstatus == "0" ]] && [[ $plugstatus == "1" ]] && [[ $charging == '-1' ]] && [[ $soc -ne 100 ]] && [[ $wakeupmyrenaultlp1 == "1" ]] ; then
      ...
      sudo python /var/www/html/openWB/modules/soc_myrenault/zoenwakelp1.py $myrenault_userlp1 $myrenault_passlp1  $myrenault_locationlp1  $myrenault_countrylp1 $soc_vinlp1
 fi
fi
