...
clearPassword(){
	...
	safeSed "s/$passwordConfigText=.*/$passwordConfigText=''/" $CONFIGFILE
}

setTokenPassword(){
	openwbDebugLog ${DMOD} 2 "Lp$CHARGEPOINT: Writing token password to config."
	safeSed "s/$passwordConfigText=.*/$passwordConfigText='$TOKENPASSWORD'/" $CONFIGFILE
	safeSed "s/$mfaPasscodeConfigText=.*/$mfaPasscodeConfigText=XXX/" $CONFIGFILE
}
