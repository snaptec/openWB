	...
	Gauge = (function (superClass) {
		...
		Gauge.prototype.getAngle = function (value) {
			if (this.options.mirror) {
				return 2 * Math.PI - ((value - this.minValue) / (this.maxValue - this.minValue)) * (this.options.range) * Math.PI;
			} else {
				return (1 + this.options.angle) * Math.PI + ((value - this.minValue) / (this.maxValue - this.minValue)) * (this.options.range) * Math.PI;
			}
		};

		Gauge.prototype.getStartAngle = function () {
			if (this.options.mirror) {
				return (2 - this.options.angle) * Math.PI;
			} else {
				return (1 + this.options.angle) * Math.PI;
			}
		};

		Gauge.prototype.getEndAngle = function () {
			if (this.options.mirror) {
				return (2 - this.options.angle - this.options.range) * Math.PI;
			} else {
				return (1 + this.options.angle + this.options.range) * Math.PI;
			}
		};
		...
		Gauge.prototype.renderStaticLabels = function (staticLabels, w, h, radius) {
			var font, fontsize, j, len, match, re, ref, rest, rotationAngle, value, textvalue, textstring, labelPosition;
			this.ctx.save();
			...
			for (j = 0, len = ref.length; j < len; j++) {
				...
				if ((!this.options.limitMin || textvalue >= this.minValue) && (!this.options.limitMax || textvalue <= this.maxValue)) {
					if ((textvalue == this.minValue && !this.options.mirror) || (textvalue == this.maxValue && this.options.mirror)) {
						this.ctx.textAlign = "left";
					} else if ((textvalue == this.minValue && this.options.mirror) || (textvalue == this.maxValue && !this.options.mirror)) {
						this.ctx.textAlign = "right";
					}
					if (textvalue != 0) {
						textvalue = formatNumber(value, staticLabels.fractionDigits);
					}
					if ( staticLabels.unit !== void 0) {
						textstring = textvalue + " " + staticLabels.unit;
					} else {
						textstring = textvalue;
					}
					if (staticLabels.insideLabel) {
						labelPosition = -radius + this.lineWidth / 2 + 6;
						this.ctx.textBaseline = "top";
					} else {
						labelPosition = -radius - this.lineWidth / 2;
						this.ctx.textBaseline = "bottom";
					}
					rotationAngle = this.getAngle(value) - 3 * Math.PI / 2;
					this.ctx.rotate(rotationAngle);
					this.ctx.fillText(textstring, 0, labelPosition);
					this.ctx.rotate(-rotationAngle);
					this.ctx.textAlign = "center";
				}
			}
		...
		Gauge.prototype.render = function () {
			...
			if (this.options.staticZones) {
				...
				this.ctx.beginPath();
				this.ctx.arc(w, h, radius, this.getStartAngle(), displayedAngle, this.options.mirror);
				this.ctx.lineWidth = this.lineWidth;
				this.ctx.stroke();
				this.ctx.strokeStyle = this.options.strokeColor;
				this.ctx.beginPath();
				this.ctx.arc(w, h, radius, displayedAngle, this.getEndAngle(), this.options.mirror);
				this.ctx.stroke();
				this.ctx.save();
				this.ctx.translate(w, h);
			}
