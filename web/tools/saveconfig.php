...
<?php
	...
	try {
		...
		if( array_key_exists( 'i3usernamelp2', $_POST ) ){
			// charge point 2
			$i3authfile = $_SERVER['DOCUMENT_ROOT']."/openWB/modules/soc_i3s1/auth.json";
			$i3auth_fp = fopen($i3authfile, 'w');
			fwrite($i3auth_fp,"{".PHP_EOL.'"username": "'.$settingsArray['i3usernamelp2'].'",'.PHP_EOL.'"password": "'.$settingsArray['i3passwortlp2'].'",'.PHP_EOL.'"vehicle": "'.$settingsArray['i3vinlp2'].'"'.PHP_EOL."}".PHP_EOL);
			fclose($i3auth_fp);
		}
