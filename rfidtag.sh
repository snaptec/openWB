...
rfid() {
	...
	if [ -f ramdisk/startRfidScanData ]; then
		...
		if (( pluggedLp > 0 )); then
			...
			if (( secondsSinceRfidScan > MaximumSecondsAfterRfidScanToAssignCp )); then
				...
				for ((currentCp=1; currentCp<=NumberOfSupportedChargePoints; currentCp++)); do
					if [[ "${pluggedLps[$currentCp]}" -eq "0" ]]; then
						echo "$NowItIs: Disabling CP #${currentCp} as it's unplugged and delay after RFID tag scan has been exceeded"
	...
	# handle un-plug
	if (( unpluggedLp > 0 )); then
		echo "$NowItIs: Charge point #${unpluggedLp} has been UNplugged - stop sending accounting data after one final transmission"

		# one final transmission of accounting data ...
		sendAccounting $unpluggedLp

		# ... before we disabled it
		rm "ramdisk/startRfidScanDataLp${unpluggedLp}"
	fi
...
setLpPlugChangeState() {
	...
	pluggedLp=0
	pluggedLp=0
	pluggedLps=($plugstat $plugstats1)
